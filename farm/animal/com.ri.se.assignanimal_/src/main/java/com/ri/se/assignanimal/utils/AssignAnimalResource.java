...
import com.ri.se.assignanimal.dto.*;
...
public class AssignAnimalResource {
	...
	private AssignAnimalEntityConverter assignanimalEntityConverter = new AssignAnimalEntityConverter();
		...
		public AssignAnimalResource(AssignAnimalService assignanimalService){
	...
	public Response get(@PathParam("aaid") final String aaid) {
		...
		try{
			return Response.status(Response.Status.OK).entity(assignanimalEntityConverter.getDTOAssignAnimal( assignanimal)).build();
	...
	@ApiResponses({ @ApiResponse(code = 200, message = "OK"),
			@ApiResponse(code = 500, message = "Internal Server error !") })
	public Response listByAnimalID(@PathParam("animalID") final String animalID) {
		List<AssignAnimal> list = this.assignanimalService.list();
		if (java.util.Objects.isNull(list)) {
			return Response.status(Response.Status.INTERNAL_SERVER_ERROR).entity(new Message("Problems when fetching list or requested data list does not exist !")).build();
		}
	...
	@ApiResponses({ @ApiResponse(code = 200, message = "OK"),
			@ApiResponse(code = 500, message = "Internal Server error !") })
	public Response post(@Valid DTOAssignAnimal dtoassignanimal) {

...
public class AssignAnimalMapper implements ResultSetMapper<AssignAnimal> {
	...
	public AssignAnimal map(int i, ResultSet resultSet, StatementContext statementContext) throws SQLException {
		return new AssignAnimal(resultSet.getString(aaid), resultSet.getTimestamp(assignedDate), resultSet.getString(notes), resultSet.getString(examiner), resultSet.getString(action), resultSet.getTimestamp(submissionDate), resultSet.getString(animals), resultSet.getString(reportReference), resultSet.getString(reportSubmitted));

...
public class AssignFeedMapper implements ResultSetMapper<AssignFeed> {
	...
	private static final String  expectedFinishDate = "expectedFinishDate";
	...
	public AssignFeed map(int i, ResultSet resultSet, StatementContext statementContext) throws SQLException {
		return new AssignFeed(resultSet.getString(afid), resultSet.getTimestamp(assignedDate), resultSet.getTimestamp(expectedFinishDate),resultSet.getString(assignedBy), resultSet.getString(fpid), resultSet.getString(assignedTo));

...
public class FarmMapper implements ResultSetMapper<Farm> {
	...
	public Farm map(int i, ResultSet resultSet, StatementContext statementContext) throws SQLException {
		return new Farm(resultSet.getString(farmId), resultSet.getString(farmName), resultSet.getString(owner), resultSet.getTimestamp(dateTime), resultSet.getString(country), resultSet.getString(notes), resultSet.getString(city), resultSet.getString(street), resultSet.getString(postalcode), resultSet.getString(farmContactNo), resultSet.getString(farmCompany), resultSet.getString(email));

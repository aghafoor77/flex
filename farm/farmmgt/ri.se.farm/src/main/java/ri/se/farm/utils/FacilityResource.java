...
public class FacilityResource {
	...
	public Response get(@PathParam("facilityId") final String facilityId) {
		Facility facility = this.facilityService.get(facilityId);
	...
	@ApiResponses({ @ApiResponse(code = 200, message = "OK"),
			@ApiResponse(code = 500, message = "Internal Server error !") })
	public Response getByFarmId(@PathParam("farmid") final String farmid) {
		List<Facility> facility = this.facilityService.getbyfarmid(farmid);
		if (java.util.Objects.isNull(facility)) {
			return Response.status(Response.Status.INTERNAL_SERVER_ERROR).entity(new Message("Problems when fetching entry or requested entry does not exist !")).build();
		}
		try{

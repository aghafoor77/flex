...
public interface GeneralHealthObservationDAO{
	@SqlUpdate("CREATE TABLE IF NOT EXISTS `GeneralHealthObservation` ( `ghoid` varchar(100) NOT NULL,`limping` varchar(100) NULL DEFAULT NULL,`observer` varchar(100) NULL DEFAULT NULL,`wound` varchar(100) NULL DEFAULT NULL,`bcs` varchar(100) NULL DEFAULT NULL,`notes` varchar(100) NULL DEFAULT NULL,`swelling` varchar(100) NULL DEFAULT NULL,`gheDate` timestamp NULL DEFAULT NULL,`animalID` varchar(100) NULL DEFAULT NULL,`wight` varchar(100) NULL DEFAULT NULL, PRIMARY KEY (`ghoid`)) ENGINE=InnoDB DEFAULT CHARSET=latin1;")
	public void create();

	@SqlUpdate("insert into GeneralHealthObservation(ghoid, limping, observer, wound, bcs, notes, swelling, gheDate, animalID, wight) values(:ghoid, :limping, :observer, :wound, :bcs, :notes, :swelling, :gheDate, :animalID, :wight)")
	public int post(@BindBean GeneralHealthObservation generalhealthobservation_);

	@SqlUpdate("UPDATE GeneralHealthObservation SET ghoid=:ghoid, limping=:limping, observer=:observer, wound=:wound, bcs=:bcs, notes=:notes, swelling=:swelling, gheDate=:gheDate, animalID=:animalID, wight=:wight WHERE ghoid=:ghoid")

...
public interface AssignFeedDAO{
	@SqlUpdate("CREATE TABLE IF NOT EXISTS `AssignFeed` ( `afid` varchar(100) NOT NULL,`assignedDate` timestamp NULL DEFAULT NULL,`assignedBy` varchar(100) NULL DEFAULT NULL,`fpid` varchar(100) NULL DEFAULT NULL,`assignedTo` varchar(100) NULL DEFAULT NULL, PRIMARY KEY (`afid`)) ENGINE=InnoDB DEFAULT CHARSET=latin1;")
	public void create();

	@SqlUpdate("insert into AssignFeed(afid, assignedDate, assignedBy, fpid, assignedTo) values(:afid, :assignedDate, :assignedBy, :fpid, :assignedTo)")
	public int post(@BindBean AssignFeed assignfeed_);

	@SqlUpdate("UPDATE AssignFeed SET afid=:afid, assignedDate=:assignedDate, assignedBy=:assignedBy, fpid=:fpid, assignedTo=:assignedTo WHERE afid=:afid")

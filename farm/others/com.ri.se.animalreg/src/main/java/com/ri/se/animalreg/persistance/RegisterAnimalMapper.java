...
public class RegisterAnimalMapper implements ResultSetMapper<RegisterAnimal> {
	...
	public RegisterAnimal map(int i, ResultSet resultSet, StatementContext statementContext) throws SQLException {
		return new RegisterAnimal(resultSet.getString(animalID), resultSet.getString(receivedFarmID), resultSet.getString(animalIDMother), resultSet.getString(notes), resultSet.getString(pregnancyExamination), resultSet.getString(sex), resultSet.getInt(weight), resultSet.getTimestamp(dateOfBirth), resultSet.getString(breed), resultSet.getString(birthPlace), resultSet.getString(employerID), resultSet.getString(unit), resultSet.getString(receivedFarmName), resultSet.getString(previousAnimalID), resultSet.getString(aboutAnimal), resultSet.getString(others), resultSet.getString(status), resultSet.getDate(registrationDate));

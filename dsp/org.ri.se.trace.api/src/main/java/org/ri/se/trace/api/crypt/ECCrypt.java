package org.ri.se.trace.api.crypt;
...
public class ECCrypt {
	...
	public static void main(String a[]) {
		ECCrypt ecCrypt = new ECCrypt();
		Credentials cred = new AccountsManager()
				.createCredentilsFromPrivateKey("0xd3ba9797ac3a53a88f927bed5ee5ea4103521a812edc9b48bc9b69a1e955cc54");
		ecCrypt.signMessage("Hello".getBytes(), cred.getEcKeyPair());
	}
	...
	public static BigInteger recoverFromSignature(int recId, ECDSASignature sig, byte[] message) {
		...
		// (mod n).
		// In the above equation ** is point multiplication and + is point addition (the
		// EC group
		// operator).
		//
		// We can find the additive inverse by subtracting e from zero then taking the
		// mod. For
		// example the additive inverse of 3 modulo 11 is 8 because 3 + 8 mod 11 = 0,
		// and
		// -3 mod 11 = 8.
	...
	/**
	 * Given an arbitrary piece of text and an Ethereum message signature encoded in
	 * bytes, returns the public key that was used to sign it. This can then be
	...
	/**
	 * Given an arbitrary message and an Ethereum message signature encoded in
	 * bytes, returns the public key that was used to sign it. This can then be
	 * compared to the expected public key to determine if the signature was
	 * correct.
	 *
	...
	/**
	 * Given an arbitrary message hash and an Ethereum message signature encoded in
	 * bytes, returns the public key that was used to sign it. This can then be
	 * compared to the expected public key to determine if the signature was
	 * correct.
	 *

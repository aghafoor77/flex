...
import com.ri.se.transferedanimal.utils.TransferedAnimalResource;
...
import com.rise.carrier.slaughterhouse.utils.CarrierSlaughterhouseResource;
import com.rise.carrier.slaughterhouse.utils.SHCarrierResource;
...
import transporter.persistance.TransporterService;
import transporter.utils.TemplateHealthCheck;
import transporter.utils.TransporterResource;
...
public class TransporterApplication extends Application<TransporterConfiguration> {
	...
	public void run(TransporterConfiguration configuration, Environment environment) throws SocketException {
		...
		TransferedAnimalService transferedanimalService = dbi.onDemand(TransferedAnimalService.class);
		transferedanimalService.create();
		TransferedAnimalResource transferedanimalResource = new TransferedAnimalResource(transferedanimalService);
		
		CarrierSlaughterhouseService carrierslaughterhouseService = dbi.onDemand(CarrierSlaughterhouseService.class);
		carrierslaughterhouseService.create();
		
		
		SHCarrierService shcarrierService = dbi.onDemand(SHCarrierService.class);
		shcarrierService.create();
		SHCarrierResource shcarrierResource = new SHCarrierResource(shcarrierService);
		CarrierSlaughterhouseResource carrierslaughterhouseResource = new CarrierSlaughterhouseResource(carrierslaughterhouseService, shcarrierService, carrierService, transferedanimalService);		
		...
		environment.jersey().register(transferedanimalResource);
		environment.jersey().register(shcarrierResource);
		environment.jersey().register(carrierslaughterhouseResource);
	}
}

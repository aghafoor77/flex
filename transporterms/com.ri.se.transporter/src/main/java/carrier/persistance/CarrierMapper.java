...
public class CarrierMapper implements ResultSetMapper<Carrier> {
	...
	public Carrier map(int i, ResultSet resultSet, StatementContext statementContext) throws SQLException {
		return new Carrier(resultSet.getString(CID), resultSet.getString(carrierNumber), resultSet.getString(notes), resultSet.getString(species), resultSet.getString(transportType), resultSet.getString(TID), resultSet.getBoolean(longDistance));

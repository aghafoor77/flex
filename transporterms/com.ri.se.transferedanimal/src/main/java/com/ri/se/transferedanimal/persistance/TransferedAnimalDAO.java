...
@RegisterMapper(TransferedAnimalMapper.class)
public interface TransferedAnimalDAO{
	...
	@SqlUpdate("UPDATE TransferedAnimal SET animalID=:animalID, animalIDMother=:animalIDMother, birthPlace=:birthPlace, previousFarmContact=:previousFarmContact, currentWeight=:currentWeight, receivedFarmName=:receivedFarmName, sex=:sex, farmId=:farmId, transferDate=:transferDate, birthDate=:birthDate, breed=:breed, transactionID=:transactionID, transactionTime=:transactionTime, currentStatus =:currentStatus WHERE animalID=:animalID")
	public int put(@BindBean TransferedAnimal transferedanimal_);

	@SqlQuery("select * from TransferedAnimal;")
	public List<TransferedAnimal> list();
	
	@SqlQuery("select * from TransferedAnimal where currentStatus ='RECEIVED'")
	public List<TransferedAnimal> listAvaiable();
	

	@SqlQuery("select * from TransferedAnimal where animalID=:animalID;")
	public TransferedAnimal get(@Bind("animalID") String animalID);

	@SqlUpdate("Delete from TransferedAnimal where animalID=:animalID;")
	public int delete(@Bind("animalID") String animalID);
	
	@SqlQuery("select MAX(transactionTime) from TransferedAnimal;")
	public long getLatest();
	
	@SqlQuery("select * from TransferedAnimal where status=:status;")
	public List<TransferedAnimal> getAnimalAsCurrentStatus(@Bind("status") String status);

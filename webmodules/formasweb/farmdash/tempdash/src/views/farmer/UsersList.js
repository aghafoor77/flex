...
/* ["#393E41", "#E94F37", "#1C89BF", "#A1D363",
	...
	fetchRegisteredAnimals() {
		let url = window.sessionStorage.getItem(endpoints.AM) + '/tapi/idms/v1/account/listusers'
		fetch(url, {
			method: "GET",
			headers: {
			...
			.catch(error => {
				this.setAlert("Connection Error-", error.message + " (listusers)")
				return false
			});
						...
						<CCardBody>
							<CDataTable
								...
								fields={[
									...
									{ label: 'Veid.', key: 'veid', _style: { width: '10%' } },
									{ label: 'Email.', key: 'email', _style: { width: '10%' } },
									{ label: 'Roles', key: 'roles', _style: { width: '15%' } }								]}
								hover
								striped
								columnFilter
								itemsPerPage={7}
								pagination
								clickableRows
								onRowClick=	{(item) => this.props.history.push(`/humanresource/assignuser/${item.veid}`)}
								scopedSlots={{
									'AssignRole':
										(item) => (
											<td>
												<CButton block color="info" id={item.email} onClick={(e) => this.handleDetailClick(e, item)}>Detail</CButton>
											</td>
										),
									'status':
										(item) => (
											<td>
												{this.getBadge("Active")}
											</td>
										),

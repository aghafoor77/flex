...
class ExaminarAssignedStatus extends Component {
  ...
  fetchAnimalAssignment() {
    ...
    fetch(url, {
      ...
      .then(res => {
        if (res.status === 200) {
          res.json()
            ...
            .catch(error => {
              this.setAlert("Error", error.message + " (Fetching animal health record )")
              return false
            });
        } else {
      ...
      .catch(error => {
        this.setAlert("Connection Error-", error.message + " (Fetching animal health record )")
        return false
      });
  }
  ...
  fetchAnimalAssignmentStatus() {
    ...
    fetch(url, {
      ...
      .then(res => {
        if (res.status === 200) {
          res.json()
            ...
            .catch(error => {
              this.setAlert("Error", error.message + " (Fetching request assignment status)")
              return false
            });
        } else {
          return this.displayError(res)
      ...
      .catch(error => {
        this.setAlert("Connection Error-", error.message + " (Fetching request assignment status)")
        return false
      });
  }
  /*put = (e) => {
            ...
            <CCardBody>
              <CDataTable
                ...
                fields={[
                  ...
                  { label: 'reportReference', key: 'reportReference', _style: { width: '20%' } }
                ]}
                hover
                striped
                columnFilter

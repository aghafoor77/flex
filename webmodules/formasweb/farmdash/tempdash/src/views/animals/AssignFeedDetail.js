...
class AssignFeedDetail extends Component {
  ...
  handleChange(event) {
		switch (event.target.id) {
			  case 'expectedFinishDate': this.setState({ expectedFinishDate: event.target.value }); break;
			  default: return '';
		}
	}
  ...
  ///
  post() {
	  if(this.state.expectedFinishDate==""){
	  alert("Please select expected finish date !")
	  return;
	  }
    let animals = [];
    ...
    fetch(url, {
      ...
      body: JSON.stringify({ data: animals, expectedFinishDate: this.state.expectedFinishDate })
    })
      .then(res => {
        if (res.ok) {
    ...
    fetch(url, {
      ...
      .then(res => {
        if (res.status === 200) {
          res.json()
            .then(data => {
            	this.setState({ expectedFinishDate : data.expectedFinishDate });            	
              data.data.map(item => {
            ...
            <CCardBody>
              <table className="table table-striped table-hover">
                <tbody>
                  ...
                  <td>
                  <CFormGroup>
                     <CInput type="date" name="expectedFinishDate" id="expectedFinishDate" onChange={this.handleChange.bind(this)} value={this.state.expectedFinishDate} placeholder="date" />
                </CFormGroup>

...
class ObservationDetail extends Component {
 ...
 fetchGAHR(val) {
		...
		let url = window.sessionStorage.getItem(endpoints.GHE)+'/application/v1/generalhealthobservation'+cri;
	    fetch(url, {
	    method: "GET",
	    headers: {
	      Accept: "application/json",
	      ...
	      .then(res => {
	        if (res.status === 200) {
	          res.json()
	            ...
	            .catch(error => {
	              this.setAlert("Server Error-", error.message + " (generalhealthobservation data fetching)")
	              return false
	            });
	        } else {
	          return this.displayError(res)
	      ...
	      .catch(error => {
	              this.setAlert("Connection Error-", error.message + " (generalhealthobservation data fetching)")
	              return false
	            });
	        ...
	        <CCard>
	          ...
	          <CCardBody>
	            <CDataTable
	              ...
	              fields={[
					  { label: 'No.', key: 'ghoid', _style: { width: '10%'} },
	              ...
	              clickableRows
	              onRowClick={(item) => {
					   						let gg = (item.ghoid.startsWith("GHE") === true ?'gahe/'+item.ghoid:'gaho/'+item.ghoid)
	              ...
	              scopedSlots={{
	                'type':
	                  (item) => (
	                    <td>
	                      {(item.ghoid.startsWith("GHE") === true ?'Examination ('+(item.reportType === null?'-':item.reportType)+")":"Observation")
											}
	                    </td>
	                  ),
	              }}

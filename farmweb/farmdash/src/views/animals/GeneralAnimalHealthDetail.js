...
class GeneralAnimalHealthDetail extends Component {
   ...
   getBadge = status => {
    ...
    switch (event.target.id) {
		case 'animalID': 
			...
			this.fetchGAHR(event.target.value);
			break;
        default: return '';
    }
  }
 ...
 fetchGAHR(val) {
		...
		let url = window.sessionStorage.getItem(endpoints.GHE)+'/application/v1/generalhealthexamination'+cri;
	    fetch(url, {
	    method: "GET",
	    headers: {
	      Accept: "application/json",
	      ...
	      .then(res => {
	        if (res.status === 200) {
	          res.json()
	            ...
	            .catch(error => {
	              this.setAlert("Server Error-", error.message + " (generalhealthexamination data fetching)")
	              return false
	            });
	        } else {
	          return this.displayError(res)
	      ...
	      .catch(error => {
	              this.setAlert("Connection Error-", error.message + " (generalhealthexamination data fetching)")
	              return false
	            });
	        ...
	        <CCard>
	          ...
	          <CCardBody>
	            <CDataTable
	              ...
	              fields={[
					  { label: 'No.', key: 'gaheid', _style: { width: '10%'} },
					  ...
					  { label: 'Wound', key: 'wound', _style: { width: '30%'} },
				      { label: 'Swelling',key: 'swelling', _style: { width: '25%'} }
	        	  ]}
	              hover
				  striped
				  columnFilter
	              ...
	              clickableRows
	              onRowClick={(item) => {
					   						let gg = (item.gaheid.startsWith("GHE") === true ?'gahe/'+item.gaheid:'gaho/'+item.gaheid)
											this.props.history.push(`/animals/healthrecords/`+gg)}
										}
	              scopedSlots={{
	                'type':
	                  (item) => (
	                    <td>
	                      {(item.gaheid.startsWith("GHE") === true ?'Examination':"Observation")
											}
	                    </td>
	                  ),
	              }}
...
export default GeneralAnimalHealthDetail
